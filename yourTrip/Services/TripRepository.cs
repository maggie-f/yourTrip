    ...
    public class TripRepository
    {
        ...
        internal IList<TripModels> Get(string userId, bool future)
        {
            using (var db = new ApplicationDbContext())
            {
                ...
                //trips.ToList();
                if(!future)
                    return db.Trips.Where(x => x.UserRefId == userId).Where(x => x.Departure < DateTime.UtcNow).OrderBy(x => x.Departure).ToList();
        ...
        internal TripModels GetNextTrip(string userId)
        {
            using(var db = new ApplicationDbContext())
            {
                TripModels trip = db.Trips.Where(x => x.UserRefId == userId).Where(x => x.Departure >= DateTime.UtcNow).OrderBy(x => x.Departure).FirstOrDefault();
